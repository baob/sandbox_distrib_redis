  ...
  context 'with PrimesList model' do
    ...
    describe '(collaboration with queued_test model)' do
      ...
      before(:each) do
        ...
        allow(queued_tests_model).to receive(:min).and_return(3)
        allow(queued_tests_model).to receive(:delete).with(3)
      end
      ...
      it 'dequeues queued tests with #min' do
        expect(queued_tests_model).to receive(:min).and_return(3)
        subject.run(2)
      end

      it 'deletes dequeued tests with #delete' do
        expect(queued_tests_model).to receive(:delete).with(3)
        subject.run(2)
      end
